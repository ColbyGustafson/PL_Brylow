...
(* handle all cases in exp datatype *)
fun typeof(VAR(x), Delta, Gamma) = 
        let val tau = find(x, Gamma) handle NotFound _ => find(x, Gamma)
        in tau
        end
  | typeof(LITERAL(v), Delta, Gamma) =
        (case LITERAL(v)
           of LITERAL(BOOLV(v))      => booltype
            | LITERAL(NUM(v))        => inttype
            | LITERAL(SYM(v))        => symtype
            | LITERAL(PAIR(v1, NIL)) => listtype(typeof(LITERAL(v1), Delta, Gamma))
            | LITERAL(PAIR(v1, v2))  =>
                  let val tyV1 = typeof(LITERAL(v1), Delta, Gamma)
                      val tyV2 = typeof(LITERAL(v2), Delta, Gamma)
                  in  if eqType(listtype tyV1, tyV2)
                      then tyV2
                      else raise TypeError "pair types do not match"
                  end
            (* check v1 with FORALL *)
            | LITERAL(NIL)           => FORALL (["'a"], listtype tvA)
            | _                      => raise TypeError "LITERAL not fully implemented yet"
        )
  | typeof(SET(x, e), Delta, Gamma) =
        let val xtype = typeof(VAR(x), Delta, Gamma)
            val etype = typeof(e, Delta, Gamma)
        in  if eqType(xtype, etype) then xtype
            else raise TypeError ("Set variable " ^ x ^ " of type " ^ typeString(xtype) ^ " to value of type " ^ typeString(etype))
        end
  | typeof(IFX(e1, e2, e3), Delta, Gamma) =
      let val e1Type = typeof(e1, Delta, Gamma)
          val e2Type = typeof(e2, Delta, Gamma)
          val e3Type = typeof(e3, Delta, Gamma)
      in
          if eqType(e1Type, booltype) then
            if eqType(e2Type, e3Type) then e2Type
            else raise TypeError ("Branches of `if` expression must have the same type")
          else
            raise TypeError ("Condition of `if` expression must have type bool")
      end
  | typeof(WHILEX(e1, e2), Delta, Gamma) =
      let val e1Type = typeof(e1, Delta, Gamma)
      in
        if eqType(e1Type, booltype)
        then unittype
        else raise TypeError ("Condition of `while` loop must have type bool, but found type " ^ typeString(e1Type))
      end
  | typeof(APPLY(f, args), Delta, Gamma) =
      let val tau_f = typeof(f, Delta, Gamma)
          val tau_args = map (fn arg => typeof(arg, Delta, Gamma)) args
          (* Building  a matching function type and check if the two types are equal *)
          (* param_types should match arg types *)
      in
        (case tau_f of
            ...
            | FORALL(["'a'"], return_type) =>
                ...
                (* in if eqTypes(test, tau_args) then return_type
                   else raise TypeError ("Function applied to arguments of incorrect types" ^ typeString test) *)
                end
  ...
  | typeof(LETX(LETSTAR, bindings, e), Delta, Gamma) =
      let
        (* Process each binding, extending the environment for each subsequent binding *)
        fun processBindings(bindings, env) =
            case bindings of
                [] => env
              | (name, exp)::bs =>
                let
                    (* Compute the type of the current expression in the current environment *)
                    val tau = typeof(exp, Delta, env)
                    (* Extend the environment with the new binding for subsequent bindings *)
                    val newEnv = bind(name, tau, env)
                in
                    (* Recursively process the rest bindings with the extended environment *)
                    processBindings(bs, newEnv)
                end

        (* Create the extended environment with all bindings processed *)
        val extendedGamma = processBindings(bindings, Gamma)

        (* Type-check the body of the LET* expression using the extended environment *)
        val bodyType = typeof(e, Delta, extendedGamma)
      in
        (* The type of the LET* expression is the type of the body *)
        bodyType
      end
  | typeof(LETX(LET, bindings, e), Delta, Gamma) =
      let
        (* Extend the environment with the new bindings *)
  ...
  | typeof(BEGIN es, Delta, Gamma) =
        if null es then
          ...
          let val bodytypes = map (fn e => typeof(e, Delta, Gamma)) es
          in List.last bodytypes
          end
  (*| typeof _ = raise TypeError "typeof" *)
  (* let, if are do not require polymorphic type stuff to be implemented *)
  
(* VAL, VALREC, EXP, DEFINE *)
fun typdef(EXP(e), Delta, Gamma) =
        let val tau = typeof(e, Delta, Gamma)
        in (Gamma, typeString(tau)) (* returns pair of gamma and typestring *)
        end
  | typdef(VAL(v, e), Delta, Gamma) =
        if not (isbound(v, Gamma)) 
        then 
          let val tau = typeof(e, Delta, Gamma)
          in  (bind(v, tau, Gamma), typeString(tau))
          end
        else raise TypeError "val is already bound"
  | typdef(VALREC(x, tau, LAMBDA(formals, body)), Delta, Gamma) =
    let
        val assumedGamma = bind(x, tau, Gamma)
        (* Type check the body of the lambda expression *)
        val bodyType = typeof(LAMBDA(formals, body), Delta, assumedGamma)
    in
        (* Ensure that the bodyType matches the expected type τ *)
        if eqType(bodyType, tau) then
            (* Update the environment with the new recursive definition *)
            (bind(x, tau, Gamma), typeString(tau))
        else
            raise TypeError ("The body of VALREC does not match the expected type")
    end
  | typdef(VALREC(x, tau, e), Delta, Gamma) =
        raise TypeError ("VALREC expects a LAMBDA expression for its body")
  | typdef(DEFINE(f, tau, (formals, body)), Delta, Gamma) =
    let
        val funType = FUNTY(List.map #2 formals, tau)
        val (fnames, ftypes) = ListPair.unzip formals
    ...
    in
        if eqType(bodyType, tau) then
            ...
            raise TypeError ("Function body type does not match declared type")
    end
  (*| typdef _ = raise TypeError "typdef" *)
  ...
  let fun addKind ((name, kind), kinds) =
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                            ...
                            (* primitive functions for \tuscheme\ [[::]] 382a *)
                             ...
                             (* FORALL is here (used for polymorphic types) *)
