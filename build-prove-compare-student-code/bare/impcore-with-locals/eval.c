...
//checks for primitive val
Value readPrim(void) {
    int inp;
    if (scanf("%d", &inp) != 1) {
        runerror("Error: invalid integer");
    }
    return inp;
}
...
/* make [[f]] the function denoted by [[e->apply.name]], or call [[runerror]] 51a */
                             ...
                             /* apply [[f.userdef]] and return the result 51c */
                                      {
                                          ...
                                          Namelist  ls = f.userdef.locals;
                                          Valuelist vs = evallist(e->
                                    apply.actuals, globals, functions, formals);
                                          checkargc(e, lengthNL(xs), lengthVL(vs
                                                                             ));

                                        Valenv envWLocals = mkValenv(xs, vs);
					  for (Namelist l = ls; l; l = l->tl){
					  	bindval(l->hd, 0, envWLocals);
					  }
                                          return eval(f.userdef.body, globals,
                                                   functions, envWLocals);
                                      }
                      case PRIMITIVE:
          ...
          /* apply \impcore\ primitive [[println]] to [[vs]] and return S299c */
           ...
           /* apply \impcore\ primitive [[printu]] to [[vs]] and return S299d */
            ...
            /* read that takes no args */
                                          else if (f.primitive == strtoname("read"))
                                              {
                                                  checkargc(e, 0, lengthVL(vs));
                                                  return readPrim();
                                              }
...
/* evaluate [[d->use]], possibly mutating [[globals]] and [[functions]] S296    c */
            {
