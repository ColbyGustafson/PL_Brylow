...
Exp reduce_to_exp(int code, struct Component *components) {
    ...
    case SUGAR(WHILESTAR): //new
        return mkWhilex(components[0].exp, mkBegin(components[1].exps)); //new
    case SUGAR(DO_WHILE): //new
        return mkBegin(mkEL(components[0].exp, mkEL(mkWhilex(components[1].exp, components[0].exp), NULL))); //new
    case SUGAR(FOR): //new
        return mkBegin(mkEL(components[0].exp, mkEL(mkWhilex(components[1].exp, mkBegin(mkEL(components[3].exp, mkEL(components[2].exp, NULL)))), NULL))); //new
    case SUGAR(CAND):
...
static ShiftFun whilestarshifts[]   = { sExp,  sExps,             stop }; //new
static ShiftFun dowhileshifts[]     = { sExp,  sExp,              stop }; //new
static ShiftFun forshifts[]         = { sExp,  sExp, sExp, sExps,  stop }; //new
...
struct ParserRow exptable[] = {
  ...
  { "while*", SUGAR(WHILESTAR), whilestarshifts}, //new
  { "do-while", SUGAR(DO_WHILE), dowhileshifts}, //new
  { "for", SUGAR(FOR), forshifts}, //new
...
struct Usage usage_table[] = {
    ...
    { BEGIN,               "(begin exp ... exp)" },
    { SUGAR(WHILESTAR),    "(while* cond exp ... exp)" }, //new
    { SUGAR(DO_WHILE),     "(do-while cond body)" }, //new
    { SUGAR(FOR),          "(for exp cond exp body)" }, //new
