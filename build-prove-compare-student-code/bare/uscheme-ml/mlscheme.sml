...
(* TO COMPILE *)
(* mosmlc -o uscheme-ml mlscheme.sml *)
...
val _ = op bindings  : (name * exp) list parser
  in usageParsers
   ...
   (* rows added to ML \uscheme's [[exptable]] in exercises ((prototype)) 322 *) (*  *)
     , ("(cond ([q a] ...))",
        let fun desugarCond qas = 
          case qas of
            []           => APPLY(VAR("error"), LITERAL(SYM("Cond: all question results were false!"))::[])
          | (q, a)::qas' => IFX(q, a, desugarCond(qas')) 
     ...
     , ("(&& e1 ...)", 
        let fun desugarAnd es =
          case es of
            []     => LITERAL(BOOLV(true))
          | e::[]  => e
          | e::es' => IFX(e, desugarAnd(es'), LITERAL(BOOLV(false)))

        val _ = op desugarAnd : exp list -> exp
     ...
     , ("(|| e1 ...)",
        let fun desugarOr es =
          case es of
            []     => LITERAL(BOOLV(false))
          | e::[]  => e
          | e::es' => LETX(LET, ("freshvar%1", e)::[], IFX(VAR("freshvar%1"), VAR("freshvar%1"), desugarOr(es')))

        val _ = op desugarOr : exp list -> exp
